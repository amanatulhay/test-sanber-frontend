...
export const meta: MetaFunction = () => {
    return [
      { title: "Fave Char List" },
      { name: "description", content: "Fave Char List" },
    ];
};

export const loader = async ({params}: LoaderFunctionArgs) => {
  try {
    const response = await fetch(`${baseUrl}person/favorite-character/${params.person_id}`);
    if (!response.ok){
      throw new Error('Failed to fetch data');
    }
    const favechars = await response.json();
    ...
    return defer ({ favechars, person });
  } catch (error) {
    console.error('Error fetching data:', error);
    return ({ favechars: [], person: null });
  }
};

export default function Person() {
    const { favechars, person } : { favechars : any, person : any } = useLoaderData();
    const navigate = useNavigate();
    const {account} = authStore();

    const handleEdit = async (id: number) => {
        navigate('/dashboard/person/' + person.id + '/favechar/' + id + '/edit');
    };

    const handlePost = async () => {
        navigate('/dashboard/person/' + person.id + '/favechar/create');
    };

    const handleDelete = async (id: number) => {
        Swal.fire({
            title: "Are you sure?",
            text: "Are you sure you want to delete this fave char?",
            icon: "warning",
            ...
            if (result.isConfirmed) {
                await axios.delete(`${baseUrl}favorite-character/${id}`, {headers: {"Authorization" : "Bearer "+ account?.token }})
                .then(() => {
                    Swal.fire({
                        title: 'Success!',
                        text: `Successfully Delete Fave Char!`,
                        icon: 'success',
                    }).then(() => {
                        navigate("/dashboard/person/" + person.id + "/favechar")      
                    });
                }).catch((e) => {
                    ...
                    Swal.fire({
                        icon: "error",
                        title: "Oops...",
                        text: "Failed to delete Fave Char.",
                    });
    ...
    const FaveCharsList = () => {
      ...
      return (
        <>
        <h1 className="text-2xl my-2 font-bold"> <FaPerson className="inline-block mr-1 mb-2"/> Data Fave Chars for {person.name}</h1>
        <hr />
        <button onClick={()=>{
                    handlePost()
                    }}
        ...
        <div className="-mx-4 sm:-mx-8 px-4 sm:px-8 py-4 overflow-x-auto">
            <div className="inline-block min-w-full shadow-md rounded-lg overflow-hidden">
                <table className="min-w-full leading-normal">
                    ...
                    <tbody>
                    {favechars?.map((item : any, i: number) => (
                        <tr key={i}>
