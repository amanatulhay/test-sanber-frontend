...
const HobbyForm = ({formType, formValues, person_id}: HobbyFormProps)=>{
    ...
    const onSubmit: SubmitHandler<FormValues> = async (data) => {
        try {    
            if (formValues?.id){
                await axios.put(`${baseUrl}hobby/${formValues.id}`,
                    {name: data.name, person_id: Number(person_id)},
                    {headers: {"Authorization": `Bearer ${account?.token}`}}
                );
            } else {
                await axios.post(`${baseUrl}hobby`,
                    {name: data.name, person_id: Number(person_id)},
                    {headers: {"Authorization": `Bearer ${account?.token}`}}
                );
            }
            Swal.fire({
                title: "SUCCESS",
                text: "Successfully submit Form Hobby!",
                icon: "success",
            });
        } catch (err){
            Swal.fire({
            ...
            >
                <div className="border-b border-stroke px-6.5 py-4 dark:border-strokedark">
                    ...
                    <div className="flex justify-between">
                        <Link to={`/dashboard/person/${person_id}/hobby`} className="my-8 text-white bg-gray-700 hover:bg-gray-800 focus:ring-4 focus:ring-gray-300 font-medium rounded-lg text-sm px-16 py-2.5 me-2 dark:bg-gray-600 dark:hover:bg-gray-700 focus:outline-none dark:focus:ring-gray-800">Back to Table</Link>                       
